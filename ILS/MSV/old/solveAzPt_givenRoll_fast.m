...
x=[0 0];
a.upx(tpi,fi) = cam.width/2 +.5 +200;
a.upy(tpi,fi) = cam.height/2+.5 +300;
pixelsc = pixel2camsc(cam, [a.upx(tpi,fi) a.upy(tpi,fi)]).*[1 d2r d2r];
u1 = fcnvec2uvec( fcnSC2CC(pixelsc)) ;
cam.tpnedhat(tpi,:) = [1 1 -1];
cam.aposteriori.ned(fi,:) = [0 0 0];
...
rpy = [roll x']*r2d;
u2 = fcnrotateW2B(rpy(1)*d2r,rpy(2)*d2r,rpy(3)*d2r,u1)*fcnrange(dx_ned);


%YAW
dx_ned = cam.tpnedhat(tpi,:)-cam.aposteriori.ned(fi,:); dx_sc = fcnelaz(dx_ned);
relaz = pixel2camsc(cam, [a.upx(tpi,fi) a.upy(tpi,fi)])*d2r;  felaz=relaz(2:3);
%dpitchyaw = (dx_sc - felaz)*r2d;


C_camned2tp1ned = fcnRPY2DCM_B2W([roll dx_sc]);
C_camcenter2campix = fcnRPY2DCM_B2W([0 felaz]);
C = C_camned2tp1ned*C_camcenter2campix';
C*u1' * fcnrange(dx_ned);

rpy2 = fcnW2BDCM2RPY(C')*r2d
